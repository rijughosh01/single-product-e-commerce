...
exports.registerUser = async (req, res, next) => {
  try {
    ...
    const verificationUrl = `${req.protocol}://${req.get('host')}/api/v1/email/verify/${verificationToken}`;
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    try {
      await sendEmail({
        email: user.email,
        subject: 'Ghee E-commerce Password Recovery',
        message
      });
...
exports.resendVerification = async (req, res, next) => {
  try {
    ...
    try {
      ...
      return next(new ErrorHandler('Verification email could not be sent', 500));
    }
  } catch (error) {
    next(error);
  }
