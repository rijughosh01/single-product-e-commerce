...
exports.getSubscription = async (req, res, next) => {
  try {
    ...
    if (subscription.user.toString() !== req.user.id) {
      return next(new ErrorHandler('Not authorized to access this subscription', 403));
    }
...
exports.updateSubscription = async (req, res, next) => {
  try {
    ...
    if (subscription.user.toString() !== req.user.id) {
      return next(new ErrorHandler('Not authorized to update this subscription', 403));
    }
...
exports.pauseSubscription = async (req, res, next) => {
  try {
    ...
    if (subscription.user.toString() !== req.user.id) {
      return next(new ErrorHandler('Not authorized to pause this subscription', 403));
    }
...
exports.resumeSubscription = async (req, res, next) => {
  try {
    ...
    if (subscription.user.toString() !== req.user.id) {
      return next(new ErrorHandler('Not authorized to resume this subscription', 403));
    }
...
exports.cancelSubscription = async (req, res, next) => {
  try {
    ...
    if (subscription.user.toString() !== req.user.id) {
      return next(new ErrorHandler('Not authorized to cancel this subscription', 403));
    }
    ...
    for (const subscription of dueSubscriptions) {
      try {
        ...
        const order = await Order.create({
          ...
          shippingPrice: 0, // Calculate based on shipping rules
